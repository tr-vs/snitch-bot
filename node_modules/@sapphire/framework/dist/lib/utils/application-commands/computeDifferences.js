...
const optionTypeToPrettyName = new Map([
    ...
    [3 /* String */, 'string option'],
    [4 /* Integer */, 'integer option'],
    [5 /* Boolean */, 'boolean option'],
    [6 /* User */, 'user option'],
    [7 /* Channel */, 'channel option'],
    [8 /* Role */, 'role option'],
    [9 /* Mentionable */, 'mentionable option'],
    [10 /* Number */, 'number option']
]);
const contextMenuTypes = [3 /* Message */, 2 /* User */];
const subcommandTypes = [2 /* SubcommandGroup */, 1 /* Subcommand */];
function getCommandDifferences(existingCommand, apiData) {
    const differences = [];
    if (existingCommand.type !== 1 /* ChatInput */ && existingCommand.type) {
        // Check context menus
        if (contextMenuTypes.includes(existingCommand.type ?? 1 /* ChatInput */)) {
...
function* reportOptionDifferences({ option, existingOption, currentIndex, keyPath = (index) => `options[${index}]` }) {
    ...
    if (subcommandTypes.includes(existingOption.type) && subcommandTypes.includes(option.type)) {
        ...
        if (castedExisting.type === 2 /* SubcommandGroup */ &&
            castedExpected.type === 2 /* SubcommandGroup */) {
        ...
        else if (castedExisting.type === 1 /* Subcommand */ &&
            castedExpected.type === 1 /* Subcommand */) {
            // 0. No existing options and now we have options
...
function hasMinMaxValueSupport(option) {
    return [4 /* Integer */, 10 /* Number */].includes(option.type);
}
function hasChoicesAndAutocompleteSupport(option) {
    return [4 /* Integer */, 10 /* Number */, 3 /* String */].includes(option.type);
}
