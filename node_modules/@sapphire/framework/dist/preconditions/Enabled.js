...
class CorePrecondition extends Precondition_1.AllFlowsPrecondition {
    ...
    messageRun(_, command, context) {
        return command.enabled
            ? this.ok()
            : this.error({ identifier: "commandDisabled" /* CommandDisabled */, message: 'This message command is disabled.', context });
    }
    chatInputRun(_, command, context) {
        return command.enabled
            ? this.ok()
            : this.error({ identifier: "commandDisabled" /* CommandDisabled */, message: 'This chat input command is disabled.', context });
    }
    contextMenuRun(_, command, context) {
        return command.enabled
            ? this.ok()
            : this.error({ identifier: "commandDisabled" /* CommandDisabled */, message: 'This context menu command is disabled.', context });
    }
}
