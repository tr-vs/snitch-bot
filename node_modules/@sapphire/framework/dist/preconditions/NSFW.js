...
class CorePrecondition extends Precondition_1.AllFlowsPrecondition {
    messageRun(message) {
        ...
        return Reflect.get(message.channel, 'nsfw') === true
            ? this.ok()
            : this.error({ identifier: "preconditionNsfw" /* PreconditionNSFW */, message: 'You cannot run this message command outside NSFW channels.' });
    }
    async chatInputRun(interaction) {
        ...
        return Reflect.get(channel, 'nsfw') === true
            ? this.ok()
            : this.error({ identifier: "preconditionNsfw" /* PreconditionNSFW */, message: 'You cannot run this chat input command outside NSFW channels.' });
    }
    async contextMenuRun(interaction) {
        ...
        return Reflect.get(channel, 'nsfw') === true
            ? this.ok()
            : this.error({ identifier: "preconditionNsfw" /* PreconditionNSFW */, message: 'You cannot run this context menu command outside NSFW channels.' });
    }
}
