...
module.exports.Get = async (interaction) => {
    ...
    let track = interaction.options.getString('track') ? interaction.options.getString('track') : null;
    let artist;
    ...
    if (track === null) {
        ...
        if (autocorrect.results.trackmatches.track[0] == undefined) {
            return interaction.followUp('Could not find the track.');
        }
    ...
    if (know1.length === 0) {
        return interaction.followUp(`No one listens to ${track} by ${artist} here.`);
    }
