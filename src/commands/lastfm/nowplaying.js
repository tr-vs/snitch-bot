...
class NowPlayingCommand extends Command {
    constructor(context, options) {
        super(context, {
            ...options,
            name: 'nowplaying',
            description: 'Show the song you are currently listening to.',
    ...
    registerApplicationCommands(registry) {
		const builder = new SlashCommandBuilder()
			.setName('lastfm')
			.setDescription('commands that show your music stats--lastfm account required')
            .addSubcommand(subcommand => 
                subcommand
                    .setName(this.name)
                    .setDescription(this.description)
                    .addUserOption(option => option.setName('user').setDescription('show the song another user is currently listening to'))
            )
		registry.registerChatInputCommand(builder, {
			behaviorWhenNotIdentical: RegisterBehavior.Overwrite,
			guildIds: ['720435717192548374'],
            idHints: ['953124775696400465']
		});
	}
            ...
            if (settings.embed === undefined || settings.embed === 1) {
				...
				if(final.track == undefined || final.track.userplaycount == undefined || album === undefined) {
					const embed = new MessageEmbed()
						...
						.setThumbnail(pic)
						.setColor('#2f3136');
                    interaction.reply({ embeds: [ embed ] }).then(int => {
